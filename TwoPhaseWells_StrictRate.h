...
namespace FVM {
    namespace TwoPhaseWellsStrict 
    {
            ...
            const RelPermParams& rp_params)
        {
            ...
            double pi_mass_sum = 0.0;   ///< Σ PI_mass_i over perforated cells (kg/s/Pa)
            double rate_diag_sum = 0.0; ///< Rate 模式井行的对角项 Σ PI_mass_i (kg/s/Pa)
            ...
            std::vector<double>& Mg_cell)
        {
            ...
            double Mtot_well = 0.0; // 统计净总质量流量（用于日志）
            double Mpos_well = 0.0; // Σ max(dotM_tot_i, 0)  (流入单元为正)
            double Mneg_well = 0.0; // Σ min(dotM_tot_i, 0)  (流出单元为负)
            ...
            // crossflow 诊断：若完井段同时存在正负贡献，说明“部分段注入 + 部分段回流/采出”
            // 这是物理上可能出现的（定压井/多完井段），但若下游模块对其做单向截断，会导致质量不守恒。
