...
constexpr const char* kPlanKey = "h_tp_co2h2o_constpp_nofrac_injprod";
...
struct TestCaseSpec {
    ...
    bool enable_control_ramp = true;
    int control_ramp_steps = 40;
    double control_ramp_min = 0.1;
...
void SyncPTSwFieldsToFM(MeshManager& mgr,
                        FieldManager_2D& fm,
                        ...
                        const std::vector<double>& swBlocks,
                        double pFallback,
                        double tFallback,
                        double swFallback) {
    const int totalBlocks = std::max(mgr.getTotalDOFCount(), 0);
    if (totalBlocks <= 0) return;

    FIM_StateMap<3> state;
    state.InitSizes(static_cast<std::size_t>(totalBlocks));
    for (int i = 0; i < totalBlocks; ++i) {
        state.P[static_cast<std::size_t>(i)] =
            (i < static_cast<int>(pBlocks.size())) ? pBlocks[static_cast<std::size_t>(i)] : pFallback;
        state.T[static_cast<std::size_t>(i)] =
            (i < static_cast<int>(tBlocks.size())) ? tBlocks[static_cast<std::size_t>(i)] : tFallback;
        state.Sw[static_cast<std::size_t>(i)] =
            (i < static_cast<int>(swBlocks.size())) ? swBlocks[static_cast<std::size_t>(i)] : swFallback;
    }

    FIM_Engine::SyncStateToFieldManager(
        state,
        fm,
        mgr,
...
const CaseCommon::CaseCatalogEntry& GetC7CatalogEntryOrThrow() {
    const CaseCommon::CaseCatalogEntry* entry = CaseCommon::FindCaseCatalogEntry("C7");
    if (!entry) throw std::runtime_error("[Test_H_TP_CO2H2O_InjProd] missing C7 catalog entry.");
    return *entry;
}

CaseCommon::CaseArtifactPaths BuildC7ArtifactPaths() {
    const CaseCommon::CaseCatalogEntry& entry = GetC7CatalogEntryOrThrow();
...
void EnsureC7ArtifactContractDirs(const CaseCommon::CaseArtifactPaths& artifacts) {
...
C7OutputPaths BuildC7OutputPaths(const CaseCommon::CaseArtifactPaths& artifacts,
...
TestCasePlan BuildPlanByKey(const std::string& key) {
    if (key != kPlanKey) {
        throw std::runtime_error("[Test_H_TP_CO2H2O_InjProd] unknown plan key: " + key);
    }
    TestCasePlan plan;
    plan.plan_key = key;
...
int FindNearestMatrixCell(const MeshManager& mgr, double targetX, double targetY) {
    const auto& cells = mgr.mesh().getCells();
    if (cells.empty()) {
        throw std::runtime_error("[Test_H_TP_CO2H2O_InjProd] mesh has no matrix cells.");
    }
    int bestIdx = -1;
    ...
    if (bestIdx < 0) {
        throw std::runtime_error("[Test_H_TP_CO2H2O_InjProd] failed to resolve nearest matrix completion.");
    }
    return bestIdx;
}
...
void WriteC7StageManifest(const CaseCommon::CaseArtifactPaths& artifacts,
                          CaseCommon::CaseStage stage,
                          const std::string& status,
                          const std::string& outputDir) {
    Case2DReferenceIO::WriteAsciiFile(
        artifacts.engineering_stage_manifest_path,
        "[Test_H_TP_CO2H2O_InjProd] failed to write C7 stage manifest",
        [&](std::ofstream& out) {
            out << "case_code=C7\n";
...
void WriteC7ReferenceContract(const TestCaseSpec& cfg,
                              const CaseCommon::CaseArtifactPaths& artifacts,
                              CaseCommon::CaseStage stage) {
    Case2DReferenceIO::WriteAsciiFile(
        artifacts.reference_contract_path,
        "[Test_H_TP_CO2H2O_InjProd] failed to write C7 reference contract",
        [&](std::ofstream& out) {
            out << "# C7 Reference Contract\n\n";
            out << "- Stage: `" << CaseCommon::ToString(stage) << "`\n";
            out << "- Reference mode: `comsol`\n";
            out << "- Validation variables: `pressure`, `temperature`, `co2_saturation`, `well_bhp`, `well_rate`, `production_temperature`\n";
            out << "- Default well policy: `injector rate + producer BHP`\n";
            out << "- Injection fluid: `co2 into brine`\n";
...
void WriteC7StageStatus(const TestCaseSpec& cfg,
                        const CaseCommon::CaseArtifactPaths& artifacts,
                        CaseCommon::CaseStage stage,
                        const std::string& status,
                        const std::string& outputDir) {
    Case2DReferenceIO::WriteAsciiFile(
        ...
        "[Test_H_TP_CO2H2O_InjProd] failed to write C7 stage status",
        [&](std::ofstream& out) {
            out << "# C7 Template Status\n\n";
            out << "- Stage: `" << CaseCommon::ToString(stage) << "`\n";
            out << "- Status: `" << status << "`\n";
            ...
            out << "## Stage semantics\n";
            out << "- `solve_only`: run the coupled N=3 injector-producer solve and write engineering outputs plus well time series.\n";
            out << "- `prepare_reference`: materialize weak-coupling reference inputs without running the solve.\n";
            out << "- `validate_only`: rerun the N=3 case under the case root and stop at `missing_reference` until COMSOL data exists.\n";
            out << "- `full_workflow`: run solve + reference check under the case root.\n";
        });
}

void WriteC7PropertyTable(const TestCaseSpec& cfg, const C7OutputPaths& paths) {
    Case2DReferenceIO::WriteAsciiFile(
        paths.property_table_path,
        "[Test_H_TP_CO2H2O_InjProd] failed to write C7 property table",
        [&](std::ofstream& out) {
            out << "key,value,unit\n";
            ...
            out << "water_rho_const," << cfg.water_props.rho << ",kg/m3\n";
            out << "water_mu_const," << cfg.water_props.mu << ",Pa*s\n";
            out << "water_cp_const," << cfg.water_props.cp << ",J/kg/K\n";
            out << "water_cv_const," << cfg.water_props.cv << ",J/kg/K\n";
            out << "water_k_const," << cfg.water_props.k << ",W/m/K\n";
            out << "co2_rho_const," << cfg.gas_props.rho << ",kg/m3\n";
            out << "co2_mu_const," << cfg.gas_props.mu << ",Pa*s\n";
            out << "co2_cp_const," << cfg.gas_props.cp << ",J/kg/K\n";
            out << "co2_cv_const," << cfg.gas_props.cv << ",J/kg/K\n";
            out << "co2_k_const," << cfg.gas_props.k << ",W/m/K\n";
            out << "p_init," << cfg.p_init << ",Pa\n";
            out << "t_init," << cfg.t_init << ",K\n";
            out << "sw_init," << cfg.sw_init << ",1\n";
...
void WriteC7WellSchedule(const std::vector<ResolvedWellSpec>& wells, const C7OutputPaths& paths) {
    Case2DReferenceIO::WriteAsciiFile(
        paths.well_schedule_path,
        "[Test_H_TP_CO2H2O_InjProd] failed to write C7 well schedule",
        [&](std::ofstream& out) {
...
void WriteC7ReferenceSpec(const TestCaseSpec& cfg,
                          const CaseCommon::CaseArtifactPaths& artifacts,
                          const C7OutputPaths& paths,
                          const std::vector<ResolvedWellSpec>& wells) {
    Case2DReferenceIO::WriteAsciiFile(
        paths.reference_spec_path,
        "[Test_H_TP_CO2H2O_InjProd] failed to write C7 reference spec",
        [&](std::ofstream& out) {
            out << "# C7 Reference Input Spec\n\n";
            out << "- Case dir: `" << artifacts.case_dir << "`\n";
            out << "- Grid: `" << cfg.nx << "x" << cfg.ny << "`\n";
            out << "- Domain: `" << cfg.lx << " m x " << cfg.ly << " m`\n";
            out << "- Variables: `pressure`, `temperature`, `co2_saturation`, `well_bhp`, `well_rate`, `production_temperature`\n";
...
void WriteC7MatlabStub(const C7OutputPaths& paths,
                       CaseCommon::CaseStage stage,
                       const CaseCommon::CaseArtifactPaths& artifacts) {
    Case2DReferenceIO::WriteAsciiFile(
        paths.matlab_script_path,
        "[Test_H_TP_CO2H2O_InjProd] failed to write C7 matlab stub",
        [&](std::ofstream& out) {
            out << "% C7 Matlab plotting placeholder\n";
            out << "% Stage: " << CaseCommon::ToString(stage) << "\n";
            out << "% Case root: " << artifacts.case_dir << "\n";
            out << "% Pending shared C/F two-phase well-aware plotting extraction.\n";
        });
}

void WriteRunSummary(const CaseRunSummary& summary, const C7OutputPaths& paths) {
    Case2DReferenceIO::WriteAsciiFile(
        paths.run_summary_path,
        "[Test_H_TP_CO2H2O_InjProd] failed to write C7 run summary",
        [&](std::ofstream& out) {
...
void WriteC7WellTimeSeries(const std::vector<WellTimeSeriesRow>& rows, const C7OutputPaths& paths) {
    Case2DReferenceIO::WriteAsciiFile(
        paths.well_timeseries_path,
        "[Test_H_TP_CO2H2O_InjProd] failed to write C7 well time series",
        [&](std::ofstream& out) {
...
void ValidateReferencePayloadOrThrow(const TestCaseSpec& cfg,
    ...
    if (!missingFiles.empty()) {
        ...
        Case2DReferenceIO::WriteAsciiFile(
            ...
            "[Test_H_TP_CO2H2O_InjProd] failed to write C7 validation summary",
            [&](std::ofstream& out) {
                out << "# C7 Validation Summary\n\n";
                out << "- Status: `missing_reference`\n";
                out << "- Missing files:\n";
                for (const auto& file : missingFiles) out << "  - `" << file << "`\n";
            });
        throw std::runtime_error("[Test_H_TP_CO2H2O_InjProd] C7 reference files are missing.");
    }
    ...
    Case2DReferenceIO::WriteAsciiFile(
        ...
        [&](std::ofstream& out) {
            ...
            out << "- Note: quantitative C7 profile/monitor/well comparison remains the next C/F extraction target.\n";
        });
}

void MaterializeC7ReferenceInputs(const TestCaseSpec& cfg,
                                  const CaseCommon::CaseArtifactPaths& artifacts,
                                  CaseCommon::CaseStage stage) {
    ...
    const C7OutputPaths paths = BuildC7OutputPaths(artifacts, artifacts.case_dir);
    WriteC7PropertyTable(cfg, paths);
    WriteC7WellSchedule(wells, paths);
    WriteC7ReferenceSpec(cfg, artifacts, paths, wells);
    WriteC7MatlabStub(paths, stage, artifacts);
}
...
FIM_Engine::TransientSolverParams BuildSolverParams(const TestCaseSpec& cfg) {
    ...
    params.enable_control_ramp = cfg.enable_control_ramp;
    params.control_ramp_steps = cfg.control_ramp_steps;
    params.control_ramp_min = cfg.control_ramp_min;
...
void AppendWellSnapshotRows(const TestCaseSpec& cfg,
                            ...
                            const FIM_Engine::WellDOFManager<3>& wellMgr,
                            MeshManager& mgr,
                            FieldManager_2D& fm,
                            const FluidPropertyEvalConfig& fluidCfg,
                            const CapRelPerm::VGParams& vg,
    ...
    for (std::size_t i = 0; i < state.P.size(); ++i) {
        ...
        state.Sw[i] = (i < swBlocks.size()) ? swBlocks[i] : cfg.sw_init;
    }

    const auto wellBhpByName = wellMgr.BuildWellBhpMap(state);
    const int totalEq = wellMgr.TotalBlocksWithWells() * 3;
...
CaseRunArtifacts RunSingleCaseCore(const TestCaseSpec& cfg, const std::string& outputDir) {
    ...
    if (artifacts.summary.n_cells <= 0) {
        throw std::runtime_error("[Test_H_TP_CO2H2O_InjProd] matrix cell count is zero.");
    }
    ...
    bcVizCtx.primary_fluid_model = VTKBCPrimaryFluidModel::Water;
    bcVizCtx.bindings.push_back(VTKBCVariableBinding{pEqCfg.pressure_field, &bcP, VTKBCTransportKind::Pressure});
    bcVizCtx.bindings.push_back(VTKBCVariableBinding{sEqCfg.saturation, &bcS, VTKBCTransportKind::Saturation});
    ...
    FIM_Engine::WellDOFManager<3> wellMgr;
    wellMgr.Setup(normalizedWellsSuperset, mgr.getTotalDOFCount());

    FIM_StateMap<3> initialState;
    ...
    wellMgr.InitWellState(initialState, cfg.p_init, cfg.t_init, cfg.sw_init, initialActiveWells);

    std::vector<double> pBlocksLatest = initialState.P;
    std::vector<double> tBlocksLatest = initialState.T;
    std::vector<double> swBlocksLatest = initialState.Sw;
    ...
    modules.on_step_accepted =
        [&](int step, double timeS, double, int newtonIters, double,
            ...
            const std::vector<double>* swVec) {
            if (step <= 0) return;
            if (!pVec.empty()) pBlocksLatest = pVec;
            if (!tVec.empty()) tBlocksLatest = tVec;
            if (swVec && !swVec->empty()) swBlocksLatest = *swVec;
            SyncPTSwFieldsToFM(
                mgr,
                fm,
                resolvedModules.fluid_property_eval,
...
void PrintPrepareReferenceSummary(const CaseCommon::CaseArtifactPaths& artifacts) {
    std::cout << "[C7] prepare_reference completed\n";
...
void RunStageByKeyImpl(const std::string& key, CaseCommon::CaseStage stage) {
    ...
    const CaseCommon::CaseArtifactPaths artifacts = BuildC7ArtifactPaths();
    EnsureC7ArtifactContractDirs(artifacts);
    WriteC7StageManifest(artifacts, stage, "started", artifacts.case_dir);
    WriteC7ReferenceContract(plan.spec, artifacts, stage);
    WriteC7StageStatus(plan.spec, artifacts, stage, "started", artifacts.case_dir);
    MaterializeC7ReferenceInputs(plan.spec, artifacts, stage);

    if (stage == CaseCommon::CaseStage::PrepareReference) {
        WriteC7StageManifest(artifacts, stage, "prepared_reference_inputs", artifacts.reference_dir);
        WriteC7StageStatus(plan.spec, artifacts, stage, "prepared_reference_inputs", artifacts.reference_dir);
    ...
    try {
        ...
        const C7OutputPaths paths = BuildC7OutputPaths(artifacts, defaultOutputDir);
        switch (stage) {
        case CaseCommon::CaseStage::SolveOnly:
            runArtifacts = RunSingleCaseCore(stageSpec, artifacts.engineering_dir);
            WriteC7WellTimeSeries(runArtifacts.well_rows, paths);
            WriteRunSummary(runArtifacts.summary, BuildC7OutputPaths(artifacts, artifacts.engineering_dir));
            WriteC7StageManifest(artifacts, stage, "completed", artifacts.engineering_dir);
            WriteC7StageStatus(stageSpec, artifacts, stage, "completed", artifacts.engineering_dir);
        ...
        case CaseCommon::CaseStage::ValidateOnly:
            ...
            WriteC7StageManifest(artifacts, stage, "completed", artifacts.case_dir);
            WriteC7StageStatus(stageSpec, artifacts, stage, "completed", artifacts.case_dir);
        ...
        case CaseCommon::CaseStage::FullWorkflow:
            ...
            return;
        default:
            throw std::runtime_error("[Test_H_TP_CO2H2O_InjProd] unsupported stage in RunStageByKeyImpl.");
        }
        ...
        WriteC7StageManifest(artifacts, stage, failureStatus, defaultOutputDir);
        WriteC7StageStatus(stageSpec, artifacts, stage, failureStatus, defaultOutputDir);
        throw;
    }
}
