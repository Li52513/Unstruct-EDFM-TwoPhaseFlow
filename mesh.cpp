...
// =========================================================
// 【旧接口】 现在有其他函数正在调用，保留以兼容，后续逐步替换
// =========================================================
// 构建网格node face cell拓扑关系
...
{     
    ...
    // ---- 通用：构造一个矩形底面几何（2D面 or 3D的底面） ----
    {
        ...
        if (useQuadBase)
        {
            ...
            gmsh::option::setNumber("Mesh.Algorithm", 4);     // 8 Quad 前沿法（更稳）
            gmsh::option::setNumber("Mesh.RecombinationAlgorithm", 2); // blossom
        }
        else
        {
    ...
    for (size_t i = 0; i < elementTypes.size(); i++)
    {
        ...
        else if (!is2D && type == 4)
        {
            ...
            for (size_t j = 0; j < numTets; j++)
            {
				...
				cell.computeCenterAndVolume_3D(nodesMap_); // 计算 Cell 的中心和体积
                cells_.push_back(cell);
            }
        }
        // 棱柱单元（type == 6）
        ...
        {
            size_t numHexs = elementTags[i].size();
            for (size_t j = 0; j < numHexs; j++)
            {
                int cellId = static_cast<int>(elementTags[i][j]);
                vector<int> cellNodeIDs;
        ...
        {         
            ...
            for (size_t j = 0; j < n; ++j) 
            {
                ...
                Cell c(cellId, ids); c.computeCenterAndVolume_3D(nodesMap_); cells_.push_back(c);
            }
        }
    }
    ...
    struct FaceAcc {
        std::vector<int> ordered;      // 有序顶点（来自某个单元的 getLocalFaces_2D 原序）
        std::vector<int> cells;        // 邻接单元 id（1 或 2）
    };
    std::unordered_map<std::vector<int>, FaceAcc, VectorHash> FacetoCellMap;

    for (const auto& cell : cells_) {
        for (const auto& faceNodeIDs_ordered : cell.getLocalFaces_2D(nodesMap_)) {
    ...
    for (auto& kv : FacetoCellMap) {
        ...
        if (face.neighborCell != -1) {
            ...
            if ((n * d) < 0) {                        // 点乘>0 说明朝向进邻居，翻转
                ...
                face.neighborCell = kv.second.cells.size() == 2 ? kv.second.cells[1] : -1;
                count = count + 1;
                ReverseID.push_back(face.id);
            }
        }
        std::cout << "\n翻转的次数为."<< count << std::endl;
        std::cout << "\n被反转的面为" << count << std::endl;
        for (int i =0;i< ReverseID.size();i++)
        {
            std::cout << ReverseID[i] << std::endl;
        }
        faces_.push_back(face);
	...
	buildFaceBins_2D(); // 构建面 bin
    gmsh::write("UnstructuredMesh-EDFM.msh");

    // =========================================================
    // [Debug] 自检：检查面法向方向一致性
    // =========================================================
    std::cout << "\n[Topology Check] Verifying Face Normal Directions..." << std::endl;
    ...
    for (const auto& face : faces_)
    {
        // 1. 跳过边界没邻居的面 (边界面的法向通常默认指向计算域外，难以通过双中心校验)
        if (face.neighborCell == -1) continue;

        internalFaceCount++;

        // 2. 获取 Owner 和 Neighbor 的中心坐标
        // 注意：需确保 cellId2index_ 已构建完毕
        int ownerIdx = cellId2index_.at(face.ownerCell);
        int neighIdx = cellId2index_.at(face.neighborCell);

        const Vector& co = cells_[ownerIdx].center;
        const Vector& cn = cells_[neighIdx].center;

        // 3. 计算 d 向量 (从 Owner 指向 Neighbor)
        Vector d = cn - co;

        // 4. 计算点积
        double dot = face.normal * d;

        // 5. 判定：标准 FVM 要求法向向外，即 dot > 0
    ...
    if (errorCount == 0)
    {
        std::cout << "  [SUCCESS] All " << internalFaceCount
            << " internal faces have OUTWARD normals (Owner -> Neighbor)." << std::endl;
    }
    else {
        std::cout << "  [FAILURE] Found " << errorCount
            << " faces with INWARD normals. Please check flip logic!" << std::endl;
    }
    std::cout << "=========================================================\n" << std::endl;
...
// 构建2D 网格
// 包含：几何构建、物理组标记、网格生成、拓扑关系构建(Node-Face-Cell)
void Mesh::BuildMesh2D(double lengthX, double lengthY, int nx, int ny, bool useQuadBase, std::string Name)
{
    // =========================================================
    // 1. 初始化与几何构建 (Geometry & Physical Groups)
    // =========================================================
    ...
    // 标记物理线 (用于边界条件)
    // 建议使用常量定义这些ID, 例如在头文件中: const int TAG_LEFT = 1;
    gmsh::model::geo::addPhysicalGroup(1, { l4 }, MeshTags::LEFT, "Left");
    gmsh::model::geo::addPhysicalGroup(1, { l2 }, MeshTags::RIGHT, "Right");
    gmsh::model::geo::addPhysicalGroup(1, { l1 }, MeshTags::BOTTOM, "Bottom");
    gmsh::model::geo::addPhysicalGroup(1, { l3 }, MeshTags::TOP, "Top");
    ...
    if (useQuadBase) {
        ...
        gmsh::option::setNumber("Mesh.RecombineAll", 0);            // 三角形
        gmsh::option::setNumber("Mesh.Algorithm", 6);               // Frontal-Delaunay
    }
    gmsh::option::setNumber("Mesh.Optimize", 1);
    ...
    // =========================================================
    // 4. 读取单元并构建 Cells (只处理 Type 2三角形 和 Type 3四边形)
    // =========================================================
    std::vector<int> elementTypes;
    std::vector<std::vector<size_t>> elementTags, nodeTagsPerElement;
    // 获取 2D 维度的单元
    ...
    for (size_t i = 0; i < elementTypes.size(); i++) {
        int type = elementTypes[i];
        if (type != 2 && type != 3) continue; // 忽略非 2D 单元
        ...
        for (size_t j = 0; j < numElems; j++) {
            ...
            for (int k = 0; k < nodesPerElem; k++)
            {
                cellNodeIDs.push_back(static_cast<int>(nodeTagsPerElement[i][nodesPerElem * j + k]));
            }
            Cell cell(cellId, cellNodeIDs);
            cell.computeCenterAndVolume_2D(nodesMap_); // 2D 计算函数
            cells_.push_back(cell);
        }
    }
    ...
    // =========================================================
    // 5. 构建 Face 拓扑 (FaceHash 算法 - 保持原有逻辑)
    // =========================================================
    struct FaceAcc {
        std::vector<int> ordered;
        std::vector<int> cells;
    };
    // 用于去重：Key 是排序后的节点 ID，Value 是原始顺序和邻接单元
    std::unordered_map<std::vector<int>, FaceAcc, VectorHash> FacetoCellMap;

    for (const auto& cell : cells_) {
        // getLocalFaces_2D 在 2D 下返回的是边的节点列表
        for (const auto& faceNodeIDs_ordered : cell.getLocalFaces_2D(nodesMap_)) {
            std::vector<int> key = faceNodeIDs_ordered;
            std::sort(key.begin(), key.end());

            auto& acc = FacetoCellMap[key];
            if (acc.ordered.empty()) acc.ordered = faceNodeIDs_ordered;
            acc.cells.push_back(cell.id);
        }
    }
    ...
    // 实例化 Faces
    faces_.clear();
    // 辅助映射：用于后续快速通过节点集合找到 Face 索引，以便打上物理组标签
    std::map<std::vector<int>, int> nodeSetToFaceIndex;

    int faceId = 1;
    for (auto& kv : FacetoCellMap) {
        const auto& orderedIDs = kv.second.ordered;
        std::vector<Vector> fCoords;
        for (int nid : orderedIDs) fCoords.push_back(nodesMap_.at(nid).coord);

        Face face(faceId, orderedIDs, fCoords);
        face.ownerCell = kv.second.cells[0];
        face.neighborCell = (kv.second.cells.size() == 2) ? kv.second.cells[1] : -1;

        // 法向一致化处理 (Owner -> Neighbor / Outward)
        if (face.neighborCell != -1) {
            const Vector& co = cells_[cellId2index_.at(face.ownerCell)].center;
            const Vector& cn = cells_[cellId2index_.at(face.neighborCell)].center;

            Vector n = face.normal;
            Vector d = cn - co; // Owner -> Neighbor (Outward)

            // 【关键修正】: 
            // 如果 n 与 d 反向 (点乘 < 0)，说明 n 指向内，必须翻转
            if ((n * d) < 0) {
                std::vector<int> revIDs = orderedIDs;
                std::reverse(revIDs.begin(), revIDs.end());

                // 【关键修正】: 坐标必须反转，否则重新构造的 Face 法向不变
                std::vector<Vector> revCoords = fCoords;
                std::reverse(revCoords.begin(), revCoords.end());

                // 重新构造 Face 以更新法向
                face = Face(faceId, revIDs, revCoords);
                face.ownerCell = kv.second.cells[0];
                face.neighborCell = kv.second.cells[1];
            }
        }

        // 记录索引，供物理组匹配使用 (Key 必须是排序后的)
        if (face.neighborCell == -1) { // 仅记录边界 Face 即可
            std::vector<int> key = orderedIDs;
            std::sort(key.begin(), key.end());
            nodeSetToFaceIndex[key] = static_cast<int>(faces_.size());
        }
    ...
    // =========================================================
    // 6. 物理组映射 (Physical Groups -> Face Tags)
    // =========================================================
    // 获取所有 1D 物理组 (即 2D 网格的边界)
    gmsh::vectorpair physGroups;
    gmsh::model::getPhysicalGroups(physGroups, 1);

    for (auto& p : physGroups) {
        int tag = p.second; // 例如 TAG_LEFT (1)
        ...
        for (int ent : entities) {
            ...
            for (size_t k = 0; k < types.size(); k++) {
                // 遍历该实体下的所有线段单元
                int numNodesPerElem = (types[k] == 1) ? 2 : 0; // Type 1 是 2-node line
                if (numNodesPerElem != 2) continue;

                for (size_t j = 0; j < tags[k].size(); j++) {
                    std::vector<int> faceNodes;
                    // 提取线段的两个节点
                    faceNodes.push_back((int)nodes[k][j * 2 + 0]);
                    faceNodes.push_back((int)nodes[k][j * 2 + 1]);
                    std::sort(faceNodes.begin(), faceNodes.end());

                    // --- 核心映射区域 ---
                    // 只要查找到这个 Face，就直接把物理组 Tag 赋给它
                    if (nodeSetToFaceIndex.count(faceNodes)) {
                        int fIdx = nodeSetToFaceIndex[faceNodes];

                        // 直接赋值 ID，不使用 string
                        faces_[fIdx].physicalGroupId = tag;
                    }
                }
            }
        }
    }
    std::cout << "2D Mesh Built. Cells: " << cells_.size() << ", Faces: " << faces_.size() << std::endl;
