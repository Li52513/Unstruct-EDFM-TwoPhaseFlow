...
namespace GeomCalculate
{
    ...
    bool segSegIntersect2D(const Vector& p, const Vector& p2, const Vector& q, const Vector& q2, Vector& out, double eps)
    {
        Vector r = p2 - p;
        Vector s = q2 - q;
    ...
    int countIntersectionsWithPolygon(const std::vector<Vector>& poly, const Vector& s0, const Vector& s1, std::vector<Vector>& hits, double eps)
    {
        hits.clear();
    ...
    void splitPolygonByLine(const std::vector<Vector>& poly, const Vector& s0, const Vector& s1, std::vector<Vector>& pos, std::vector<Vector>& neg, double eps)
    {
        pos.clear(); neg.clear();
        const size_t n = poly.size();
        if (n < 3) return;
    ...
    {
        Vector se = e - s;
        double segLenSq = se.m_x * se.m_x + se.m_y * se.m_y + se.m_z * se.m_z;
    ...
    bool pointOnSegment2D(const Vector& P, const Vector& A, const Vector& B, double eps)
    {
    ...
    bool pointInPolygon2D(const Vector& P, const std::vector<int>& polyNodeIDs, const std::unordered_map<int, Node>& nodes, double eps)
    {
