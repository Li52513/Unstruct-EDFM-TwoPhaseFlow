...
{
    /**
     * @brief 基于严格 Peaceman 两相井模型，构建水/气两相的单元质量源项场 [kg/s]。
     *
     * 约定：Q>0 表示流入单元（注入），Q<0 表示流出单元（采出）。
     *
     * @param Qw_name  输出水相源场名（可为空：不输出）
     * @param Qg_name  输出气相源场名（可为空：不输出）
     */
