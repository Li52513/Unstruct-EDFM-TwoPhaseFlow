...
TestCaseSummary RunCase(const TestCaseSpec& cfg) {
    ...
    if (!convergenceLog.good()) {
        throw std::runtime_error("[Test_H_CO2_VaryPP] failed to open convergence log: " + summary.convergence_log_path);
    }
    ...
    if (nCells == 0) {
        throw std::runtime_error("[Test_H_CO2_VaryPP] matrix cell count is zero");
    }
...
void ExecutePlanByKey(const std::string& key) {
    ...
    if (it == registry.end()) {
        throw std::runtime_error("[Test_H_CO2_VaryPP] unknown registry key: " + key);
    }
