/**
 * @file Test_2D_EDFM_H_CO2_VaryPP_SingleFrac_NoWell.cpp
 * @brief Standalone test: 2D single-phase CO2 variable-property (EOS), single-fracture, no-well.
 */
...
namespace Test_H_CO2_VaryPP_SingleFrac {
namespace {
...
struct TestCaseSpec {
    ...
    double fracture_kn = 1.0e-19;
    double fracture_phi = 0.30;
...
TestCasePlan BuildDefaultPlan() {
    TestCasePlan plan;
    plan.plan_key = "h_co2_varypp_singlefrac_nowell";
    return plan;
}
...
TestCaseSummary RunCase(const TestCaseSpec& cfg) {
    ...
    if (!convergenceLog.good()) {
        throw std::runtime_error("[Test_H_CO2_VaryPP_SingleFrac] failed to open convergence log: " + summary.convergence_log_path);
    }
    ...
    if (nCells == 0) {
        throw std::runtime_error("[Test_H_CO2_VaryPP_SingleFrac] matrix cell count is zero");
    }
...

    {
        ...
        if (fRho && !fRho->data.empty()) {
            ...
            if (rMin <= rMax) {
                summary.rho_min = rMin;
                summary.rho_max = rMax;
            }
        }
...
void ExecutePlanByKeyImpl(const std::string& key) {
    ...
    if (it == registry.end()) {
        throw std::runtime_error("[Test_H_CO2_VaryPP_SingleFrac] unknown registry key: " + key);
    }
    ...
    std::cout << "\n============================================\n";
    std::cout << "[Test_H_CO2_VaryPP_SingleFrac] run completed\n";
    ...
    if (summary.probe_left_count <= 0 || summary.probe_right_count <= 0) {
        std::cout << "  [WARN] barrier probe invalid (insufficient left/right samples).\n";
    } else if (std::isfinite(summary.delta_p_cross_norm) && std::abs(summary.delta_p_cross_norm) < 1.0e-3) {
        std::cout << "  [WARN] weak cross-fracture pressure contrast: |delta_p_cross_norm| < 1e-3.\n";
    }
    std::cout << "============================================\n";
}
...
void RunTestCase() {
    ExecutePlanByKeyImpl("h_co2_varypp_singlefrac_nowell");
}
